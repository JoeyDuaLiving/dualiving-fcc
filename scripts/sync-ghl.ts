// Thin CLI wrapper around POST /api/sync/ghl - see scripts/sync-buildxact.ts
...
async function main() {
  console.log(`Triggering GHL sync via ${baseUrl}/api/sync/ghl ...`);
  const response = await fetch(`${baseUrl}/api/sync/ghl`, { method: "POST" });
